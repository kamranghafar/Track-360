...
from dashboard.models import Resource, Project, ProjectResource, SprintCycle, OATReleaseCycle, WeeklyMeeting, WeeklyProjectUpdate, Quarter, QuarterTarget, QuarterTargetResource, WeeklyProductMeeting, WeeklyProductUpdate
import random
from datetime import timedelta, date
import decimal

class Command(BaseCommand):
    help = 'Adds specific dummy data to the database for testing with predefined resources and products'

    def handle(self, *args, **options):
        self.stdout.write('Adding specific dummy data...')
        ...
        # Create Resources with specific names from the issue description
        resource_names = [
            'Zakir Muhammad Yousuf', 'Usama Ali Sheikh', 'Samiullah', 'Zain Ali Khan', 'Muhammad Hasan',
            'Shaheer', 'Yaseen', 'Shamaim Shaikh', 'Shafaq', 'Sarang',
            'Talha Shahab', 'Mahham Batool', 'Muzammil Qamar', 'Farhan Ahmed Sheikh', 'Unzela Saleem',
            'Ajmal Syed', 'Ifrah Ali', 'Fawad Qamar', 'Umer Mansoor', 'Rakesh Kumar',
            'Humza', 'Saqib', 'Aisha', 'Muhammad Bilal', 'Saira Qadir'
        ]
        ...
        for name in resource_names:
            # Create email from name (handle single names)
        ...
        # Create Projects (Products) with specific names from the issue description
        product_names = [
        ...
        for name in product_names:
            ...
            product, created = Project.objects.get_or_create(
                name=name,
                defaults={
                    'description': f'This is a product: {name}',
            ...
            if created:
                ...
                # Assign 3-8 random resources to each product to create more data
        ...
        self.stdout.write(self.style.SUCCESS(f'Created {products_created} products with resource assignments'))

        # Create Weekly Meetings (more than in the original script to generate more data)
        meetings_created = 0
        for i in range(8):  # Create 8 weekly meetings
            ...
            if created:
                ...
                # Add updates for 5-8 random products
                num_products = random.randint(5, 8)
                products = random.sample(list(Project.objects.all()), min(num_products, Project.objects.count()))
        ...
        self.stdout.write(self.style.SUCCESS(f'Created {meetings_created} weekly meetings with product updates'))

        # Create Weekly Product Meetings
        product_meetings_created = 0
        for i in range(8):  # Create 8 weekly product meetings
            meeting_date = timezone.now() - timedelta(days=i*7)  # One meeting per week

            meeting, created = WeeklyProductMeeting.objects.get_or_create(
                meeting_date=meeting_date,
                defaults={
                    'title': f"Weekly Product Meeting {meeting_date.strftime('%Y-%m-%d')}",
                    'notes': f"Product meeting notes for {meeting_date.strftime('%Y-%m-%d')}",
                    'is_completed': True
                }
            )

            if created:
                product_meetings_created += 1
                ...
                for product in products:
                    timeline_choices = ['immediate', 'short', 'medium', 'long']
                    
                    WeeklyProductUpdate.objects.create(
                        meeting=meeting,
                        project=product,
                        notes=f"General notes about {product.name}",
                        problems=f"Some identified problems with {product.name}",
                        expected_solution=f"Proposed solutions for {product.name}",
                        solution_timeline=random.choice(timeline_choices)
                    )

        self.stdout.write(self.style.SUCCESS(f'Created {product_meetings_created} weekly product meetings with updates'))
        ...
        for quarter_number in range(1, 5):
            ...
            if created:
                ...
                # Create quarter targets for 5-8 random products
                num_products = random.randint(5, 8)
                products = random.sample(list(Project.objects.all()), min(num_products, Project.objects.count()))

                for product in products:
                    # Create a quarter target for this product
                    target_descriptions = [
        ...
        # Count total rows of data created
        total_rows = (
            Resource.objects.count() +
            Project.objects.count() +
            ProjectResource.objects.count() +
            WeeklyMeeting.objects.count() +
            WeeklyProjectUpdate.objects.count() +
            WeeklyProductMeeting.objects.count() +
            WeeklyProductUpdate.objects.count() +
            Quarter.objects.count() +
            QuarterTarget.objects.count() +
            QuarterTargetResource.objects.count()
        )
        
        self.stdout.write(self.style.SUCCESS(f'Successfully added specific dummy data. Total rows created: {total_rows}'))